...
The examples \cref{fig:local-global-bigdiff,fig:local-global-mostdiff} which have a high difference between the local and global simplifications can be solved optimally using our heuristic as \cref{fig:local-global-bigdiff} is \(\varepsilon\)-positive using the given \(\varepsilon\). the polyline in \cref{fig:local-global-mostdiff} is not necessarily positive but can still be solved optimally because only three solution intervals are required to find the solution. The starting interval can never be narrowed by our construction and the second and third interval end in the last point of the polyline which can never be removed. For further investigations into optimality criteria for the heuristic we differentiate polylines into two categories.
...
\begin{lemma}
  Let \(P\) be a polyline, \(\varepsilon > 0\) and \(G\) be the global shortcut graph of \(P\) using \(\varepsilon\). \(P\) is \(\varepsilon\)-neutral if any of these following conditions hold.
	\begin{enumerate}
		...
		\item There is a global simplification that only uses positive intervals.
	\end{enumerate}
\end{lemma}
...
\begin{proof}
	\begin{enumerate}
	  \item Already shown previously.
		...
		\item If such a simplification exists, we can always proceed from the end of each solution interval to the end of the next one. As solution intervals are only shrunk from the start and never removed, the end of each solution interval is never removed. Thus, the simplification is still found.
	\end{enumerate}
\end{proof}
