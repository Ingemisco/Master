...
This exact problem has not been studied extensively, but a similar problem has been considered by \citeauthor{progressive_simplification_buchinetal}~\cite{progressive_simplification_buchinetal}. They use the local Fréchet distance to compute \emph{progressive} simplifications. Such simplifications have the property that they are subsequences of each other, meaning that any simplifications in a progressive simplification can be gotten from any other one by either only removing vertices or only adding vertices. Their minimization goal is the simplification size sum. 

Their approach is useful as the simplifications at different levels of \(\varepsilon\) are consistent. However, this comes at the cost of runtime. The continuous version requires \(\O(n^5)\) time and the discrete setting requires \(\O(n^3 m)\) time when minimizing for \(m\) levels.

We consider the simpler, non-progressive setting which allows us to solve these problems in runtime \(\O(n^4 \log n)\) for the continuous version in the global setting and \(\O(n^3 \log^2 n)\) in the local one. The discrete setting without the progressive property can be solved simply by computing the simplifications at each level and thus it is trivially possible to solve in \(\O(n^3 m)\) in the global setting and \(\O(n^2 \log n m)\) in the local setting by using suitable simplification algorithms. Thus, we cover only the global setting. 
...
Interestingly, this suggests that in high dimensions distance functions such as \(\delta_4\) can be better than both the Manhattan and Chebyshev distance as the amount of events does not scale with the dimension. In practice, \(\delta_4\) is inferior to \(\delta_2\) in every regard and \(\delta_1\) and \(\delta_\infty\) have the advantage of allowing fast, stable and exact solution algorithms. Further, high dimensions are rarely used.
...
In the local case, both Fréchet and Hausdorff, the algorithm can also be improved as there are, in fact, only quadratically many events which are exactly the distances of the shortcuts to the respective subpolyline. This is independent of the distance used and only quadratically many shortcuts can exist. This improves the search through the events to a runtime of \(\O(f(n)n\log n)\) where \(\O(f(n))\) is the runtime for a local simplification. In total, this allows also for runtimes of \(\O(n^3\log n)\) generally or \(\O(n^3\log^2 n)\) for the Euclidean, Chebyshev, and Manhattan distance in two dimensions using appropriate algorithms.
