...
Although we have not explained it in full detail, we can improve \cref{thm:euclidean_implicit_simplification_simple} by using the algorithm from \citeauthor{polyline_simplification_has_cubic_complexity_bringmannetal}. We have shown how the such a rewrite works extensively and we provide \cref{tab:explicit-implicit-translation} as a guide on how to each individual operation needs to be transformed to obtain an implicit algorithm.  
