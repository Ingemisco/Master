...
inline float squared_euclidean_distance(Point const &point1,
                                        Point const &point2) {
#if DEBUG
...
float euclidean_distance(Point const &point1, Point const &point2) {
  return std::sqrt(squared_euclidean_distance(point1, point2));
}
...
inline float unnormalized_distance(Point const &point1, Point const &point2,
                                   int p) {
#if DEBUG
  assert_compatible_points(point1, point2);
  if (p > 0) {
...
float normalized_distance(Point const &point1, Point const &point2, int p) {
  return std::pow(unnormalized_distance(point1, point2, p), 1.0 / p);
}
...
float first_reachable_euclidean(Polyline const &polyline1,
  ...
  float const dist_u_v1 = squared_euclidean_distance(v1, u);
  ...
  float const a0 = squared_euclidean_distance(v2, v1) - epsilon_squared;
  float a1 = 0;
