...
When it comes to practical implementations, it would be interesting to know if the Manhattan or Chebyshev decisions can be implemented in linear runtime instead of \(d\log d\). We are not certain whether there are linear algorithms for the explicit solutions, we have no knowledge of such algorithms nor of lower bounds that disprove them so improving the explicit solution might be a way but if this is not possible the implicit approach could still allow it. An easier, albeit tedious further improvement is to adapt the algorithms for Manhattan and Chebyshev to avoid divisions in the implicit case. Similarly to the Euclidean distance, we expect it to be possible by replacing them by multiplications in the comparisons. 
